...
public class JdbsDaoPersonneMysql implements DAO<PERSONNE> {
	...
	public PERSONNE read(String id) throws PersonneDoncExistException {
		...
		try (Connection connect = DriverManager.getConnection(dburl)) {
			PreparedStatement prepare = connect.prepareStatement("SELECT * FROM personne WHERE nom = ?  ");
			prepare.setString(1, id);
			ResultSet result = prepare.executeQuery();

			if (result.next()) {
				personne = new PERSONNEBuilder(result.getString("nom"), result.getString("prenom"))
						.dateNaissance(LocalDate.parse("1997-04-22")).fonction(result.getString("fonction")).build();
				result.close();
			} else {
		...
		try (Connection connect = DriverManager.getConnection(dburl)) {
			...
			if (!res.next()) {
				throw new PersonneDoncExistException(
						"" + "La personne que vous voulez modifier" + " n'éxiste pas :( !");
			} else {
				PreparedStatement prepare = connect.prepareStatement(
						"UPDATE personne SET prenom = ?, " + "fonction = ?, " + "datenaisssance = ? WHERE nom = ?");
	...
	public void delete(PERSONNE obj) throws PersonneDoncExistException {
		...
		try (Connection connect = DriverManager.getConnection(dburl)) {
			...
			if (!res.next()) {
				...
				PreparedStatement prepare = connect.prepareStatement("DELETE FROM personne " + "WHERE nom = ?");
				prepare.setString(1, obj.getNom());
				int result = prepare.executeUpdate();
				assert result == 1;
			}
